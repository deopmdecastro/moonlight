...
export default function CartDrawer({ open, onClose }) {
  ...
  const whatsappMessage = () => {
    ...
    const lines = items.map((i) => `• ${i.name} x${i.qty} = ${(i.price * i.qty).toLocaleString("pt-AO")} Kz`).join("\n");
    const msg = `Olá Moonlight! Gostaria de encomendar:\n\n${lines}\n\nTotal: ${total.toLocaleString("pt-AO")} Kz`;
    window.open(`https://wa.me/244927215851?text=${encodeURIComponent(msg)}`, "_blank");
  };
          ...
          <>
              ...
              {items.map((item) => (
                <div key={item.id} className="flex gap-4 p-4 bg-muted rounded-xl">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-mono text-xs text-primary mt-1">{item.price.toLocaleString("pt-AO")} Kz</p>
                  </div>
            ...
            <div className="border-t border-border pt-4 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-display text-2xl text-primary">{total.toLocaleString("pt-AO")} Kz</span>
              </div>
              <button
                onClick={whatsappMessage}
                className="w-full font-mono text-xs uppercase tracking-widest py-3.5 bg-primary text-primary-foreground rounded-full hover:bg-primary/90 transition-all duration-300"
              >
